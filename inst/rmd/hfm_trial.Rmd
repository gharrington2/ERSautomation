...
In May 2023, the Ministry of Housing, Communities and Local Government launched a new framework with a new set of metrics to ensure rough sleeping is **prevented** wherever possible, and where it does occur it is **rare**, **brief**, and **non-recurring**. This is the third quarterly publication of the framework and covers monthly information up to June 2024. 
...
label_range <- label_range %>%
  mutate(xmid = as.Date((as.numeric(xmin) + as.numeric(xmax)) / 2, origin = "1970-01-01"))

 figure_1 <- ggplot(new_df, aes(x = Date)) +
   geom_line(aes(y = `New snapshot`, color = "Snapshot_new"), size = 1.4, group = 1) +
   geom_point(aes(y = `New snapshot`), color = "#012169", size = 3) +  # Add points for Snapshot_new
   geom_line(aes(y = `New month`, color = "Month_new"), size = 1.4, group = 1) +
   geom_point(aes(y = `New month`), color = "#95B3D7", size = 3) +
  geom_rect(data = label_range, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, group = Year),
             fill = "white", color = NA, alpha = 0, inherit.aes = FALSE) +
  geom_text(data = label_range, aes(x = xmid, y = ymin -150 , label = Year, group = Year),
   ...
   scale_y_continuous(breaks = seq(0,3500, by = 500),labels = comma, limits = c(-150,3500))+
   scale_x_date(labels = scales::date_format("%b"), date_breaks = "1 month", limits = c(min_date, max_date), expand = c(0, 10)) +
   theme(
    axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0.5, family = "Arial", color = "#636363", size = 11),
...
figure_2 <- ggplot(donut_data, aes(ymax=ymax, ymin=ymin, xmax=5, xmin=3.25, fill=Institutions)) +
  ...
  geom_segment(data=subset(donut_data, Institutions == "UK armed forces"), aes(x=4, y=original_label_position, xend=5.45, yend=custom_labelPosition), color="#767171", size=0.5) +
  coord_polar(theta="y") +
  xlim(c(0, 6)) +
  ...
  theme(
    ...
    legend.title = element_blank(),
     legend.margin = margin(t = -25, b = 0, l = 0, r = 0),  # Adjusts the margin around the legend
    legend.box.margin = margin(t = -25, b = 10, l = 0, r = 0))+  # Adjusts the margin between the legend and plot area)+
...
figure_3 <- ggplot(institutions_data, aes(x = Date)) +
  geom_line(aes(y = `Prison (adult or youth)`, color = "Prison (adult or youth)"), size = 1.4, group = 1) +
  geom_point(aes(y = `Prison (adult or youth)`), color = "#000000", size = 3) +
  geom_line(aes(y = `Other justice accommodation`, color = "Other justice accommodation"), size = 1.4, group = 1) +
  geom_point(aes(y = `Other justice accommodation`), color = "#12239E", size = 3) +
  geom_line(aes(y = `General or Psychiatric Hospital`, color = "General or Psychiatric Hospital"), size = 1.4, group = 1) +
  geom_point(aes(y = `General or Psychiatric Hospital`), color = "#118DFF", size = 3) +
  geom_line(aes(y = `UK Armed Forces`, color = "UK Armed Forces"), size = 1.4, group = 1) +
  geom_point(aes(y = `UK Armed Forces`), color = "#A0D1FF", size = 3) +
  geom_line(aes(y =  `Asylum Support`, color = "Asylum Support"), size = 1.4, group = 1) +
  geom_point(aes(y =  `Asylum Support`), color = "#E6E6E6", size = 3) +
  geom_line(aes(y = `Under 25 care leavers`, color = "Under 25 care leavers"), size = 1.4, group = 1) +
  geom_point(aes(y = `Under 25 care leavers`), color = "#EFB5B9", size = 3) +
  geom_rect(data = label_range_3, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, group = Year),
             fill = "white", color = NA, alpha = 0, inherit.aes = FALSE) +
  geom_text(data = label_range_3, aes(x = xmid, y = ymin -50, label = Year, group = Year),
            color = "#636363", size = 3.5, angle = 0, hjust = 0.5, inherit.aes = FALSE) +
  dluhc_style(size = 1.5)+
  ...
  theme(
    axis.text.x = element_text(angle = 0, hjust = 1, vjust = 0.5, family = "Arial", color = "#636363", size = 10),
    axis.text.y = element_text(family = "Arial", color = "#636363", size = 10),
    panel.grid.major = element_line(color = "#F2F2F2", size = 0.3, linetype = "solid"),   
    panel.grid.major.y = element_line(color = "#F2F2F2", size = 0.3, linetype = "solid"),
    text = element_text(family = "Arial", color = "#7F7F7F"),
    ...
    legend.position = "bottom",
    legend.title = element_blank(),
    legend.margin = margin(t = -5, b = 0, l = 0, r = 0),  # Adjusts the margin around the legend
    legend.box.margin = margin(t = -5, b = 10, l = 0, r = 0),  # Adjusts the margin between the legend and plot area)+
    legend.text = element_text(family = "Arial", color = "#7F7F7F", size = 10)) +
  guides(fill = guide_legend(override.aes = list(shape = 21, size = 5))) +
  scale_color_manual(values = c("Prison (adult or youth)" = "#000000", "Other justice accommodation" = "#12239E", "General or Psychiatric Hospital" = "#118DFF", "UK Armed Forces" = "#A0D1FF", "Asylum Support" = "#E6E6E6", "Under 25 care leavers" = "#EFB5B9")) 
...
knitr::include_graphics("~/GitHub/ERSautomation/inst/rmd/images/figure_3.svg")
...
label_range_4 <- label_range_4 %>%
 ...
 figure_4 <- ggplot(df, aes(x = Date)) +
   geom_line(aes(y = `Snapshot`, color = "Snapshot"), size = 1.4, group = 1) +
   geom_point(aes(y = `Snapshot`), color = "#012169", size = 3) +  # Add points for Snapshot_new
   geom_line(aes(y = `Month`, color = "Month"), size = 1.4, group = 1) +
   geom_point(aes(y = `Month`), color = "#95B3D7", size = 3) +
   geom_rect(data = label_range_4, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, group = Year),
             fill = "white", color = NA, alpha = 0, inherit.aes = FALSE) +
   geom_text(data = label_range_4, aes(x = xmid, y = ymin -300 , label = Year, group = Year),
            color = "#636363", size = 3.5, angle = 0, hjust = 0.5, inherit.aes = FALSE) +
   labs(title = NULL, x = "Date", y = "Number of people") +
   theme(legend.position = "none") +
   scale_color_manual(values = c("Snapshot" = "#012169", "Month" = "#95B3D7")) +
   dluhc_style(size = 1.5) +
   scale_y_continuous(breaks = seq(0,9000, by = 1000),labels = comma, limits = c(-300,9000))+
   scale_x_date(labels = scales::date_format("%b"), date_breaks = "1 month", expand =c(0,10))+ #expand = c(0, 1)) +THIS IS IF YOU WANT TO REMOVE EXTRA DATA POINTS
   theme(
    axis.text.x = element_text(angle = 90, hjust = 0, vjust = 0.5, family = "Arial", color = "#636363", size = 10),
    axis.text.y = element_text(family = "Arial", color = "#636363", size = 10),
    panel.grid.major = element_line(color = "#F2F2F2", size = 0.3, linetype = "solid"),   
    panel.grid.major.y = element_line(color = "#F2F2F2", size = 0.3, linetype = "solid"),
   axis.ticks = element_line(color = "#F2F2F2", size = 0.3), 
...
figure_5 <- ggplot(comb_last_year_long, aes(x = Date, y = Value, fill = Category)) +
  ...
  geom_text(aes(label = scales::comma(Value)),
            ...
            family = "Arial",
            size = 3.2) +
  geom_rect(data = label_range_5, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, group = Year),
             fill = "white", color = NA, alpha = 0, inherit.aes = FALSE) +
  geom_text(data = label_range_5, aes(x = xmid, y = ymin -300 , label = Year, group = Year),
            color = "#636363", size = 3.5, angle = 0, hjust = 0.5, inherit.aes = FALSE) +
  labs(title = NULL, x = "Date", y = "Value") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  dluhc_style(size = 1.5) +
  scale_y_continuous(breaks = seq(0,9000, by = 1000),labels = comma, limits = c(-300, 8000))+
  scale_x_date(labels = scales::date_format("%b"), date_breaks = "1 month", expand = c(0,0)) +
  theme(
    axis.text.x = element_text(angle = 0, hjust = 1, vjust = 0.5, family = "Arial", color = "#636363", size = 10),
    axis.text.y = element_text(family = "Arial", color = "#636363", size = 10),
    panel.grid.major = element_line(color = "#F2F2F2", size = 0.3, linetype = "solid"),   
  ...
  theme(legend.position = "bottom", legend.direction = "horizontal",legend.key.size = unit(0.3, "cm"),
        legend.text = element_text(family = "Arial", color = "#7F7F7F", size = 10)) +
