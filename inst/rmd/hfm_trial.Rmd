...
input_path <- "Q:/ASCT/P005 Homelessness/008 Rough Sleeping/RS MI R/New monthly tables script/Git_Version/Publication_tables/Publication_tables241111_sn_wrong_rates.xlsx"
...
# AF colours, have added some extra
af_colours <- c("#12436D",
             "#28A197",
             "#801650",
             "#F46A25",
...
knitr::include_graphics(paste0("images/indicator_breakdown_pub_graphic_all.png"))
...
knitr::include_graphics(paste0(output_folder, "/figure_1.svg"))
...
knitr::include_graphics(paste0(output_folder, "/figure_2.svg"))
...
knitr::include_graphics(paste0(output_folder, "/figure_3.svg"))
...
knitr::include_graphics(paste0(output_folder, "/figure_4.svg"))
...
There were `r get("this_month_df_accommodation", envir = env1)` people sleeping rough over the month who were accommodated over the course of the month (`r round((get("this_month_df_accommodation", envir = env1) /  get("this_month_df_month", envir = env1))*100, digits = 0)`% of total people sleeping rough). This was `r sign(get("monthly_change_df_accommodation", envir = env1))` `r get("monthly_change_df_accommodation", envir = env1)` people, or `r round(abs((get("monthly_change_df_accommodation", envir = env1))/get("last_month_df_accommodation", envir = env1))*100, digits = 0)`%, since `r last_month`. OR `r sign(get("quarterly_change_df_accommodation", envir = env1))` `r get("quarterly_change_df_accommodation", envir = env1)` people, or `r round(abs((get("quarterly_change_df_accommodation", envir = env1))/get("last_quarter_df_accommodation", envir = env1))*100, digits = 0)`% since `r last_quarter`.
...
r2_row <- df_accommodation[1, 5:ncol(df_accommodation)] %>% 
  mutate(across(everything(), as.numeric),
         indicator = "People accommodated over the month")

r1_row <- df_month[1, (ncol(df_month) - n_months_r2 + 1):ncol(df_month)] %>%
  mutate(across(everything(), as.numeric),                                                                                                     indicator = "Total people sleeping rough over the month")

r2_graph_data <- bind_rows(r2_row, r1_row) %>%
   pivot_longer(cols = -indicator, names_to = "month")

ggplot() +
  geom_col(r2_graph_data, mapping = aes(x = month, y = value, fill = indicator), width = 0.75, position = "dodge") + 
...
```{r figure 12, echo=FALSE, warning = FALSE, message = FALSE}
...
**Figure X: Number of nights people seen sleeping rough over the last 6 months, September 2024**
...
b2_graph_data <- data.frame(
  "nights_seen" = c("1 night",
                    ...
                    "10 or more\nnights",
                    "Unknown\nnumber of\nnights"),
  "value" = c((as.numeric(df_1_night[[this_month]][1]) / r1_month) * 100,
              (as.numeric(df_2_nights[[this_month]][1]) / r1_month) * 100,
              (as.numeric(df_3_5_nights[[this_month]][1]) / r1_month) * 100,
              (as.numeric(df_6_10_nights[[this_month]][1]) / r1_month) * 100,
              (as.numeric(df_10_more_nights[[this_month]][1]) / r1_month) * 100,
              (as.numeric(df_unknown_nights[[this_month]][1]) / r1_month) * 100)
              ) %>%
...
b2_figure <- ggplot() +
  geom_col(b2_graph_data, mapping = aes(x = nights_seen, y = value), width = 0.75) + 
  dluhctheme::dluhc_style()
  # ylab("People sleeping rough\n(% of all people sleeping rough)") +
  # xlab("Number of nights seen") + 
  # theme(
  #   plot.title = ggplot2::element_text(size = size * 11, face = "bold"),
  #   plot.subtitle = ggplot2::element_text(size = size * 8),
  ...
  #   legend.text = ggplot2::element_text(size = size * 8, color = "#222222"),
  #   axis.title = ggplot2::element_text(size = size * 9, color = "#222222"),
  #   axis.title.y = element_text(margin = margin(r = 15)),
  #   axis.title.x = element_text(margin = margin(t = 5)),
  #   axis.text = ggplot2::element_text(size = size * 8, color = "#222222"),
...
ggsave("images/b2_bar.svg")
ggsave("images/b2_bar.png")
...
There were `r get("this_month_df_returners", envir = env1)` people returning to sleeping rough over the month in `r this_month` (`r round((get("this_month_df_returners", envir = env1) /  get("this_month_df_month", envir = env1))*100, digits = 0)`% of total) - `r sign(get("quarterly_change_df_returners", envir = env1))` `r round((get("quarterly_change_df_returners", envir = env1) /  get("last_quarter_df_returners", envir = env1))*100, digits = 0)`% since `r last_quarter` and `r sign(get("annual_change_df_returners", envir = env1))` `r round(((get("annual_change_df_returners", envir = env1))/get("last_year_df_returners", envir = env1))*100, digits = 0)`%  since `r last_year`.

* Of these, `r get("this_month_df_prev_in_accomm", envir = env1)` (`r round((get("this_month_df_prev_in_accomm", envir = env1) /  get("this_month_df_returners", envir = env1))*100, digits = 0)`%) were returning to sleeping rough after moving into settled accommodation in the preceding 12 months. 
...
```{r figure 5, echo=FALSE, warning = FALSE, message = FALSE}

knitr::include_graphics(paste0(output_folder, "/figure_5.svg"))
...
new_combo_data <- bind_rows(new_combo_data,
  ...
  mutate(
    indicator = forcats::fct_recode(indicator,
                                    # renaming - top 4 are the same
                                    "Rough sleeping over the month" = "Rough sleeping over the month",
                                    "Institutions" = "Institutions",
                                    "Long term" = "Long term", 
                                    "1 night" = "1 night", 
                                    "Returners" = "Returners",
                                    "New to rough sleeping" = "Month new",
                                      "Returning after settled accommodation" = "Prev in accomm",
                                      "Accommodated" = "Accommodation")) %>%
  mutate(indicator = forcats::fct_relevel(indicator,
        "Rough sleeping over the month",
        "New to rough sleeping",
        "Institutions",
        "Long term",
        "1 night",
        "Returners",
        "Returning after settled accommodation",
        "Accommodated"))

new_combo_graph <- ggplot() +
  geom_col(new_combo_data, mapping = aes(x = month, y = perc, fill = indicator), position = "dodge") +
  dluhctheme::dluhc_style() + 
  scale_fill_manual(values = af_colours)


ggsave("images/new_combo_graph.png", new_combo_graph)
ggsave("images/new_combo_graph.svg", new_combo_graph)
...
knitr::include_graphics(paste0(output_folder, "/England_map.svg"))
#knitr::include_graphics(system.file("rmd/inst/rmd/images/.svg",  package = "ERSautomation"))
knitr::include_graphics(paste0(output_folder, "/London_map.svg"))
...
knitr::include_graphics(paste0(output_folder, "/England_map_rate.svg"))
knitr::include_graphics(paste0(output_folder, "/London_map_rate.svg"))
...
```{r nat_imm_graphs, echo = FALSE, warning = FALSE, message = FALSE}
...
ggsave("images/nat_imm_chart.png", nat_imm_chart)
ggsave("images/nat_imm_chart.svg", nat_imm_chart)

knitr::include_graphics("images/nat_imm_chart.svg")
