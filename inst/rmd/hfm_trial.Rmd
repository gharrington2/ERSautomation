...
march24_month_new_eng <- as.numeric(df_month_new[1, "March 2024"])
march23_month_new_eng <- as.numeric(df_month_new[1, "March 2023"])
...
**`r march24_month_new_eng`** new people sleeping rough over the month in March 2024 (`r round(abs((march24_month_new_eng / march24_month_eng)*100),digits = 0)`% of total) - `r up_down(march24_month_new_eng, march23_month_new_eng)` `r round(abs(((march24_month_new_eng - march23_month_new_eng) / march23_month_new_eng)*100), digits = 0)`% on same period last year.
...
#### Rare (R1)
**`r march24_month_eng`** people sleeping rough over the month in March 2024 - up `r round(abs(((march24_month_eng - march23_month_eng)/march23_month_eng)*100), digits = 0)`% on March last year, and up `r round(abs(((march24_month_eng - dec23_month_eng)/dec23_month_eng)*100), digits = 0)`% since `r last_quarter`. `r march24_month_eng_rate` people per 100,000 sleeping rough over the month in March 2024 - up from `r march23_month_eng_rate` last year and `r dec23_month_eng_rate` in December 2023.
...
colnames(transposed) <- c("Prison", "Other justice accommodation", "Hospital", "UKAF", "Asylum", "Under 25 care leavers", "Date")
institutions_data <- transposed %>%
  mutate(Date = as.Date(Date, format = "%Y_%m_%d"),
         ...
         UKAF = as.numeric(UKAF),
         Asylum = as.numeric(Asylum),
         `Under 25 care leavers` = as.numeric(`Under 25 care leavers`))

#Figure 3
figure_3 <- ggplot(institutions_data, aes(x = Date)) +
  geom_line(aes(y = Prison, color = "Prison"), size = 1.1, group = 1) +
  geom_point(aes(y = Prison), color = "#000000", size = 2) +
  ...
  geom_line(aes(y = Hospital, color = "Hospital"), size = 1.1, group = 1) +
  geom_point(aes(y = Hospital), color = "#118DFF", size = 2) +
  geom_line(aes(y = UKAF, color = "UKAF"), size = 1.1, group = 1) +
  geom_point(aes(y = UKAF), color = "#A0D1FF", size = 2) +
  geom_line(aes(y = Asylum, color = "Asylum"), size = 1.1, group = 1) +
  geom_point(aes(y = Asylum), color = "#7F7F7F", size = 2) +
  ...
  scale_color_manual(values = c("Prison" = "#000000", "Other justice accommodation" = "#12239E", "Hospital" = "#118DFF", "UKAF" = "#A0D1FF", "Asylum" = "#7F7F7F", "Under 25 care leavers" = "#EFB5B9")) +
