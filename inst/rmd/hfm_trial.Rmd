---
title: "Rough Sleeping Data Framework, X 2024"
output: 
  word_document:
...
input_path <- "Q:/ASCT/P005 Homelessness/008 Rough Sleeping/RS MI R/New monthly tables script/Git_Version/Publication_tables/Publication_tables241107.xlsx"
...
# this_month <- max(as.Date(names(df_month), "%Y_%m_%d"), na.rm = T)
# last_month <- format(lubridate::rollback(this_month - lubridate::dmonths(), roll_to_first = T), "%Y_%m_%d") 
# last_quarter <- format(lubridate::rollback(this_month - lubridate::dmonths(3), roll_to_first = T), "%Y_%m_%d")
# last_year <- format(lubridate::rollback(this_month - lubridate::dyears(), roll_to_first = T), "%Y_%m_%d")
# this_month <- format(this_month, "%Y_%m_%d")
#   
...
**`r get("this_month_df_accommodation", envir = env1)`** people who were sleeping rough over the month in `r this_month` were accommodated during the month, `r sign(get("quarterly_change_df_accommodation", envir = env1))` `r round(abs((get("quarterly_change_df_accommodation", envir = env1))/get("last_quarter_df_accommodation", envir = env1))*100, digits = 0)` % since `r last_quarter` AND `r sign(get("monthly_change_df_accommodation", envir = env1))` `r round(abs((get("monthly_change_df_accommodation", envir = env1))/get("last_month_df_accommodation", envir = env1))*100, digits = 0)`% since `r last_month`.
...
::: {custom-style="GreyBox"}
**Date of next publication**: It is expected that the next quarterly publication of monthly data up to December, will be published in February 2025.
::: 
...
* The number of people sleeping rough over the course of the month who have moved into accommodation by the end of the month, known as R2. 
...
* The number of people sleeping rough over the course of the month who are returning to sleeping rough after moving into settled accommodation, known as NR2. 
...
There were `r get("this_month_df_snapshot_new", envir = env1)` new people sleeping rough on a single night in `r this_month` (`r round(((get("this_month_df_snapshot_new", envir = env1))/get("this_month_df_snapshot", envir = env1))*100, digits = 0)`% of total people sleeping rough on a single night) - `r sign(get("quarterly_change_df_snapshot_new", envir = env1))` `r round(((get("quarterly_change_df_snapshot_new", envir = env1))/get("last_quarter_df_snapshot_new", envir = env1))*100, digits = 0)`% since `r last_quarter` and `r sign(get("annual_change_df_snapshot_new", envir = env1))` `r round(((get("annual_change_df_snapshot_new", envir = env1))/get("last_year_df_snapshot_new", envir = env1))*100, digits = 0)`% on the same period last year. 
...
There were `r get("this_month_df_institutions", envir = env1)` people sleeping rough over the month in `r this_month` who had left an institution (`r round(((get("this_month_df_institutions", envir = env1))/get("this_month_df_month", envir = env1))*100, digits = 0)`% of total). Of these people
...
**Figure 4: People sleeping rough, October 2020 to `r this_month` (R1)**
...
#new_colnames <- sapply(colnames(df_month)[5:length(df_month)], convert_to_date)
...
There were `r get("this_month_df_accommodation", envir = env1)` people sleeping rough over the month who were accommodated by the end of the month (`r round((get("this_month_df_accommodation", envir = env1) /  get("this_month_df_month", envir = env1))*100, digits = 0)`% of total people sleeping rough). This was `r sign(get("monthly_change_df_accommodation", envir = env1))` `r get("monthly_change_df_accommodation", envir = env1)` people, or `r round(abs((get("monthly_change_df_accommodation", envir = env1))/get("last_month_df_accommodation", envir = env1))*100, digits = 0)`%, since `r last_month`. OR `r sign(get("quarterly_change_df_accommodation", envir = env1))` `r get("quarterly_change_df_accommodation", envir = env1)` people, or `r round(abs((get("quarterly_change_df_accommodation", envir = env1))/get("last_quarter_df_accommodation", envir = env1))*100, digits = 0)`% since `r last_quarter`.

**Figure 5: People sleeping rough who have moved into accommodation, June 2024 to `r this_month` (R2)**
...
There were `r get("this_month_df_month", envir = env1)` people sleeping rough over the month in `r this_month`. Of these people  

* `r get("this_month_df_1_night", envir = env1)` people (`r round(((get("this_month_df_1_night", envir = env1))/get("this_month_df_month", envir = env1))*100, digits = 0)`% of total) had been seen sleeping rough 1 night over the previous 6 months 

* `r get("this_month_df_2_nights", envir = env1)` people (`r round(((get("this_month_df_2_nights", envir = env1))/get("this_month_df_month", envir = env1))*100, digits = 0)`% of total) had been seen sleeping rough 2 nights over the previous 6 months  

* `r get("this_month_df_3_5_nights", envir = env1)` people (`r round(((get("this_month_df_3_5_nights", envir = env1))/get("this_month_df_month", envir = env1))*100, digits = 0)`% of total) had been seen sleeping rough 3 to 5 nights over the previous 6 months 

* `r get("this_month_df_6_10_nights", envir = env1)` people (`r round(((get("this_month_df_6_10_nights", envir = env1))/get("this_month_df_month", envir = env1))*100, digits = 0)`% of total) had been seen sleeping rough 6 to 10 nights over the previous 6 months 

* `r get("this_month_df_10_more_nights", envir = env1)` people (`r round(((get("this_month_df_10_more_nights", envir = env1))/get("this_month_df_month", envir = env1))*100, digits = 0)`% of total) had been seen sleeping rough more than 10 nights over the previous 6 months 

* `r get("this_month_df_unknown_nights", envir = env1)` people (`r round(((get("this_month_df_unknown_nights", envir = env1))/get("this_month_df_month", envir = env1))*100, digits = 0)`% of total) had been seen sleeping rough an unknown number of nights over the previous 6 months 


**Figure 12: Number of nights people seen sleeping rough over the last 6 months June 2024 to `r this_month` (B2)**
...
Of the `r get("this_month_df_returners", envir = env1)` people sleeping rough over the month in `r this_month` who had returned to  sleeping rough, `r get("this_month_df_prev_in_accomm", envir = env1)` people (`r round((get("this_month_df_prev_in_accomm", envir = env1) /  get("this_month_df_returners", envir = env1))*100, digits = 0)`% of total people sleeping rough) were returning to sleeping rough after moving into settled accommodation in the preceding 12 months. 



Of these, (when further filtered to exclude those who hadn't been seen in more than 5 years) `r get("this_month_df_prev_in_accomm", envir = env1)` people (`r round((get("this_month_df_prev_in_accomm", envir = env1) /  get("this_month_df_returners", envir = env1))*100, digits = 0)`%) were returning to sleeping rough after moving into settled accommodation in the preceding 12 months. 

**Figure 7: People returning to sleeping rough (NR1) including those returning to sleeping rough after moving into settled accommodation (NR2), May 2023 to `r this_month`**
...
if (!dir.exists(output_folder)) {
  dir.create(output_folder, recursive = TRUE)}  # specify desired folder path 
output_file <- file.path(output_folder, "figure_7.svg")
output_file2 <- file.path(output_folder, "figure_7.png")

ggplot2::ggsave(filename = output_file, plot = figure_7, device = "svg", width = 8, height = 6)
ggplot2::ggsave(filename = output_file2, plot = figure_7, width = 8, height = 6, dpi = 300)
...
knitr::include_graphics(system.file("rmd/rmd/images/figure_7.svg", package = "ERSautomation"))

```


``` {r 7a-Figure 7a NR1_NR22, include=FALSE}

# Bind into one df
NRa_all <- dplyr::bind_rows(
  as.vector(df_returners[1, c(18:length(df_returners))]),
  as.vector(df_prev_in_accomm[1, ]) 
) %>%
  dplyr::select(-c(5:8))
...
num_months_nra <- ncol(NRa_all)

dates <- seq(from = start_date, by = "month", length.out = num_months)
formatted_dates <- format(dates, "%Y_%m_%d")
colnames(NRa_all) <- formatted_dates

# Wrangling to get into the desired format
transposed <- data.frame(t(NRa_all))
transposed <- dplyr::mutate(transposed, "Date" = rownames(transposed))
rownames(transposed) <- NULL
colnames(transposed) <- c("NR1", "NR2", "Date")
...
label_range_7a <- label_range_7a %>%
  dplyr::mutate(xmid = as.Date((as.numeric(xmin) + as.numeric(xmax)) / 2, origin = "1970-01-01"))

# Separate data frames for each label 
NR1a_label <- data.frame( Date = as.Date("2024-07-01"), 
                         y = 1100, 
                         label = "Number of people returning to \n sleeping rough (NR1)" ) 

NR2a_label <- data.frame( Date = as.Date("2024-07-01"), 
                         y = 450, 
                         label = "Number of people returning to rough sleeping \n afrer moving into settled accommodation (NR2)" )

# Figure 7a
figure_7a <- ggplot2::ggplot(NRa_data, ggplot2::aes(x = Date)) +
  ggplot2::geom_line(ggplot2::aes(y = NR1, color = "NR1"), linewidth = 1.4, group = 1) +
  ggplot2::geom_point(ggplot2::aes(y = NR1), color = "#95B3D7", size = 3) +
  ggplot2::geom_line(ggplot2::aes(y = NR2, color = "NR2"), linewidth = 1.4, group = 1) +
  ggplot2::geom_point(ggplot2::aes(y = NR2), color = "#012169", size = 3) +
  # Adding labels directly to the lines
 ggplot2::geom_rect(data = label_range_7a, ggplot2::aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, group = Year),
                     fill = "white", color = NA, alpha = 0, inherit.aes = FALSE) +
     ggplot2::geom_text(data = label_range_7a, ggplot2::aes(x = xmid, y = ymin - 300, label = Year, group = Year),
                     color = "#636363", size = 3.5, angle = 0, hjust = 0.5, inherit.aes = FALSE) +
    ggplot2::geom_text(data = NR1a_label, ggplot2::aes(x = Date, y = y, label = label),
                     color = "#012169", vjust = -0.5, 
                     # hjust = -3.6, 
                     size = 3.5) +
  ggplot2::geom_text(data = NR2a_label, ggplot2::aes(x = Date, y = y, label = label),
                     color = "#012169", vjust = -0.5, 
                     # hjust = -2.25, 
                     size = 3.5) +
  ggplot2::labs(title = NULL, x = "Date", y = "Number of people") +
...
if (!dir.exists(output_folder)) {
  dir.create(output_folder, recursive = TRUE)}  # specify desired folder path 
output_file <- file.path(output_folder, "figure_7a.svg")
output_file2 <- file.path(output_folder, "figure_7a.png")

ggplot2::ggsave(filename = output_file, plot = figure_7a, device = "svg", width = 8, height = 6)
ggplot2::ggsave(filename = output_file2, plot = figure_7a, width = 8, height = 6, dpi = 300)


```


```{r figure 7a, echo=FALSE,}

knitr::include_graphics(system.file("rmd/rmd/images/figure_7a.svg", package = "ERSautomation"))
...
knitr::include_graphics(system.file("rmd/inst/rmd/images/England_map.svg",  package = "ERSautomation"))
...
**Figure 10: Map of rate of people sleeping rough over the month per 100,000 people (R1), `r this_month`**


```{r figure 10 - map, echo=FALSE,}
# 
...
X people sleeping rough over the month of September were UK nationals, X% of the total number of people sleeping rough. X people (X%) were EU nationals, and X (X%) were non-EU nationals, with the rest having an unknown nationality. 

Figure 


Of those non-UK EEA or non-EEA nationals sleeping rough over the course of this month, X (X%) had a status that allowed for access to public funds and X (X%) had leave to enter/remain with restricted eligibility to public funds. i.e., either had pre-settled status or limited leave to remain subject to a no recourse to public funds condition.  
...
This is an estimate of the number of people seen sleeping rough over the month who have moved into accommodation by the end of the month, not including those accommodated under severe weather emergency protocols (SWEP). 
