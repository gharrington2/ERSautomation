...
perc_LAS_increases_r1_month <- (n_increases_r1_month / 296) * 100
...
# there might be a mismatch between number of cols in month and snapshot, so remove older months to make them start at the same date

excess_months <- dplyr::setdiff(names(snapshot_tot), names(month_tot))

snapshot_tot <- snapshot_tot %>%
  select(-all_of(excess_months)) # now should have the same cols as month
...
figure_4 <- ggplot2::ggplot(df, ggplot2::aes(x = Date)) +
  ...
  ggplot2::geom_text(data = df[1, ], ggplot2::aes(x = Date, y = `Snapshot`, label = " Number of people \n sleeping rough on \n a single night (R1)"),
    color = "#012169", vjust = -0.8, hjust = -4.1, size = 3.5
  ) +
  ggplot2::geom_text(data = df[1, ], ggplot2::aes(x = Date, y = `Month`, label = "   Number of people \n sleeping rough over \n       the month (R1)"),
    color = "#012169", vjust = -2.3, hjust = -2.55, size = 3.5
  )
